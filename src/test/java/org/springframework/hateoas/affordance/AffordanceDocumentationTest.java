...
public class AffordanceDocumentationTest {
	...
	// end::employee-controller[]
		...
		public Resources<Resource<Employee>> all() {
			...
			List<Resource<Employee>> employees = EMPLOYEES.values().stream()
				.map(employee -> findOne(String.valueOf(employee.getId())))
		...
		public ResponseEntity<?> newEmployee(@RequestBody Employee employee) {
			...
			employee = employee.withId(newEmployeeId);

			EMPLOYEES.put(newEmployeeId, employee);

			try {
				return ResponseEntity
					.created(new URI(findOne(String.valueOf(newEmployeeId)).getRequiredLink(Link.REL_SELF).getHref()))
					.build();
		...
		public ResponseEntity<?> updateEmployee(@RequestBody Employee employee, @PathVariable String id) {
			...
			employee = employee.withId(Integer.parseInt(id));
			...
			try {
				return ResponseEntity
					.noContent()
					.location(new URI(findOne(id).getRequiredLink(Link.REL_SELF).getHref()))
					.build();
			} catch (URISyntaxException e) {
				return ResponseEntity.badRequest().body(e.getMessage());
			}
